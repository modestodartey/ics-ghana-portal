...
function isInternalPortalPath(pathname: string) {
  return pathname.startsWith("/admin") || pathname.startsWith("/student") || pathname.startsWith("/staff");
}

export function SiteHeader() {
  const pathname = usePathname();
  const isInternalPortal = isInternalPortalPath(pathname);
  ...
  return (
    <header className="border-b border-brand-100/90 bg-white/92 backdrop-blur-xl">
      ...
      <div className={`mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 ${isInternalPortal ? "py-3" : "py-4"}`}>
        {isInternalPortal ? (
          <div className="flex flex-col gap-3 rounded-[1.3rem] border border-brand-100/80 bg-white/85 p-3 shadow-soft sm:flex-row sm:items-center sm:justify-between">
                  ...
                  {mainNavigation.map((item) => (
                    <li key={item.href}>
                      {item.href === "/login" ? (
                        ...
                        <Link
                          href={item.href}
                          className={`block rounded-full border px-4 py-2 text-sm font-medium ${
                            pathname === item.href
                              ? "border-brand-200 bg-white text-brand-700"
                              : "border-transparent bg-brand-50 text-slate-700 hover:border-brand-200 hover:bg-white hover:text-brand-700"
                          }`}
                        >
                          {item.label}
                        </Link>
